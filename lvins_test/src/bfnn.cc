...
std::vector<int> bfnn_point_k(const PointCloud::Ptr &cloud, const Vec3f &point, int k) {
    struct IndexAndDis {
        IndexAndDis() = default;
    ...
    std::sort(index_and_dis.begin(), index_and_dis.end(), [](const auto &d1, const auto &d2) {
        return d1.dis2_ < d2.dis2_;
    });
    std::vector<int> ret;
    std::transform(index_and_dis.begin(), index_and_dis.begin() + k, std::back_inserter(ret), [](const auto &d1) {
        return d1.index_;
    });
    return ret;
}
