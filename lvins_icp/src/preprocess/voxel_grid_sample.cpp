...
RawPointCloud::Ptr adaptiveVoxelGridSample(const RawPointCloud::ConstPtr &point_cloud, float init_voxel_grid_size,
    ...
    // 循环调用体素网格采样，并以两倍放大体素网格尺寸，直到点云数量小于期望值
    RawPointCloud::Ptr ret;
    auto voxel_grid_size = init_voxel_grid_size;
    while (voxel_grid_size < max_voxel_grid_size) {
        ret = voxelGridSample(point_cloud, voxel_grid_size);
        if (ret->size() < desire_point_cloud_size) {
            break;
        }
        voxel_grid_size *= 2.0f;
    }

    // 若需要返回最终体素网格尺寸，则赋值
    if (final_voxel_grid_size) {
        *final_voxel_grid_size = voxel_grid_size;
    }
