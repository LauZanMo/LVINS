 ...
 *          用法示例：<br/>
 *          1. 读取（反序列化）：auto nn_searcher = YAML::get<NearestNeighborSearcher::sPtr>(node, "nn_searcher0");<br/>
 *          2. 写入（序列化）：node["nn_searcher0"] = nn_searcher;<br/>
 *          详细实例可查看NearestNeighborSearcher::loadFromYaml的实现
 */
template<>
struct convert<NearestNeighborSearcher::sPtr> {
    /**
     * @brief 序列化NearestNeighborSearcher类指针
     * @param nn_searcher NearestNeighborSearcher的实例指针
     * @return 是否序列化成功
     */
    static Node encode(const NearestNeighborSearcher::sPtr &nn_searcher);
     ...
     * @return 是否反序列化成功
     */
    static bool decode(const Node &node, NearestNeighborSearcher::sPtr &nn_searcher);
};
